...
当前多人匹配、推送通知和正式联网同步仍在开发中。好友搜索/申请与账号资料编辑已接入后端内测接口；通知页面暂保持现状，后续再评估改为弹窗。内测反馈可以提交到 GitHub Issues，或直接联系项目维护者。

### 明日重点开发计划

1. 重构好友与群组逻辑：完善好友申请、同意/拒绝、删除好友、群组成员和群主权限，并让“找时间”只使用已同意的好友。
2. 实现本地多账号隔离：Room 日程、标签、主题和游客数据按账号分区，切换账号后不能读取其他账号的本地日程。
3. 设计异地登录管理：每台设备使用独立会话，支持查看和退出其他设备；修改密码后使旧会话失效。
...
- [ ] 重构好友申请、好友关系和群组成员逻辑。
- [ ] 实现 Room 本地数据按账号隔离，支持同一设备登录多个账号。
- [ ] 增加异地登录设备管理和会话失效机制。
