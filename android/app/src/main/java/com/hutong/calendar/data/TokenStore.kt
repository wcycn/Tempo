...
class TokenStore(context: Context) {
    ...
    fun cachedUser(): UserProfile? {
        ...
        return UserProfile(id, prefs.getString(KEY_USER_NAME, "") ?: "", email = prefs.getString(KEY_USER_EMAIL, null), accountId = prefs.getString(KEY_ACCOUNT_ID, id) ?: id, phone = prefs.getString(KEY_PHONE, null), hobbies = prefs.getString(KEY_HOBBIES, null), signature = prefs.getString(KEY_SIGNATURE, null))
    }
    fun clear() { prefs.edit().remove(KEY_TOKEN).remove(KEY_USER_ID).remove(KEY_USER_NAME).remove(KEY_USER_EMAIL).remove(KEY_ACCOUNT_ID).remove(KEY_PHONE).remove(KEY_HOBBIES).remove(KEY_SIGNATURE).apply() }
